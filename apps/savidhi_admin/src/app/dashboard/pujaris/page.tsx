...
export default function PujarisPage() {
  ...
  return (
    <div>
      ...
      {/* Edit Modal */}
      <Modal open={!!editing} onClose={() => setEditing(null)} title={isNew ? 'Add Pujari' : `Edit Pujari ${editing?.id?.slice(0, 8)}`} statusBadge={!isNew ? <StatusBadge status="ACTIVE" className="text-status-completed" /> : undefined} wide>
        {editing && (
